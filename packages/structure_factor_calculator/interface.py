...
class StructureFactorCalculator:
    ...
    def is_initialized(self):
        """
        Checks if the calculator has been successfully initialized.
    ...
    def set_hkl_input_mode(self, mode):
        """
        Sets the HKL input mode.
    ...
    def set_hkl_user_defined(self, h, k, l):
        """
        Sets a single HKL value for calculation (for HKL mode 0).
        ...
        Raises:
            TypeError: If HKL mode is not 0.
        """
        if self._hkl_mode != 0:
        ...
        print(f"User-defined HKL set to: ({h},{k},{l})")

    def set_hkl_list(self, hkl_list=None):
        """
        Sets the list of HKL values for calculation (for HKL mode 1).
        If no list is provided, a default list is used.

        Args:
            hkl_list (list, optional): A list of HKLs, where each HKL is a list or tuple of 3 integers.
                                       Defaults to None, which uses a predefined list.
        Raises:
            TypeError: If HKL mode is not 1.
            ValueError: If the provided hkl_list is not in the correct format.
        """
        if self._hkl_mode != 1:
